 ...
 *
 * @author: Jeevan Lal & (Zahid Al Haris->http request scheme authorization : Bearer)
 * @version: 0.0.5
 */
...
class Authorization_Token
{
   ...
   * ( 1 Hour ) : 60 * 60     = 3600
   * ( 8 Hour ) : 60 * 60 * 8 = 3600 jam kerja 
   */
  protected $token_expire_time = 32400;
    ...
    if (!empty($headers) and is_array($headers)) {
      foreach ($headers as $header_name => $header_value) {
        if (strtolower(trim($header_name)) == strtolower(trim($this->token_header))) {
          if ($header_value && preg_match('/Bearer\s*(\S+)\b/i', $header_value, $matches)) {
            $token = $matches[1];
          }
        }
      }
